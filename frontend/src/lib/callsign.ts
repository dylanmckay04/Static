 ...
 * Same callsign string always produces the same clean SVG.
 */

// ── UTF-8 String Hashing (Standard 32-bit FNV-1a) ──────────────────────────
...
// ── Glyph alphabet ────────────────────────────────────────────────────────
// 20 shortwave/signal-themed paths defined in a 0-20 local coordinate space.
const GLYPHS: string[] = [
...
// ── Shape outlines ─────────────────────────────────────────────────────────
type Shape = { outline: string; cx: number; cy: number; innerR: number; dotR: number };

const SHAPES: Shape[] = [
  { // circle
    outline: 'M30,4 A26,26 0 1,1 29.99,4 Z',
    cx: 30, cy: 30, innerR: 18, dotR: 31.5,
  },
  { // triangle
    outline: 'M30,4 L54,48 L6,48 Z',
    cx: 30, cy: 32, innerR: 13, dotR: 29.0,
  },
  { // pentagon
    outline: (() => {
      const pts = Array.from({ length: 5 }, (_, i) => {
        const a = (i * 72 - 90) * (Math.PI / 180);
        return `${(30 + 26 * Math.cos(a)).toFixed(1)},${(30 + 26 * Math.sin(a)).toFixed(1)}`;
      });
      return 'M' + pts.join(' L') + ' Z';
    })(),
    cx: 30, cy: 31, innerR: 15, dotR: 31.0,
  },
  { // hexagon
    outline: (() => {
    ...
    cx: 30, cy: 30, innerR: 16, dotR: 31.0,
  },
];

// Place dots neatly outside the geometric structure boundaries
function perimeterDots(shape: Shape, count: number): Array<{ x: number; y: number }> {
  return Array.from({ length: count }, (_, i) => {
    const a = (i / count) * Math.PI * 2 - Math.PI / 2;
    return {
      x: shape.cx + shape.dotR * Math.cos(a),
      y: shape.cy + shape.dotR * Math.sin(a),
    };
  });
}
...
export function generateCallsignSvg(callsign: string, size = 48): string {
  ...
  const glyphElements = chosenGlyphs.map((d, i) => {
    // Structural layout logic
    const sc = 0.50 + rng() * 0.20; // Scale glyph down slightly to comfortably prevent boundary clipping
    const rot = (rng() - 0.5) * 24; // Micro-rotations to preserve glyph legibility
    
    // Balanced horizontal layout distribution for double-glyph setups
    let ox = shape.cx - 10;
    let oy = shape.cy - 10;
    
    if (glyphCount === 2) {
      const spacingOffset = 6;
      ox += (i === 0 ? -spacingOffset : spacingOffset);
      // Small structural jitter
      ox += (rng() - 0.5) * 2;
      oy += (rng() - 0.5) * 3;
    } else {
      // Single glyph center jitter
      ox += (rng() - 0.5) * (shape.innerR * 0.3);
      oy += (rng() - 0.5) * (shape.innerR * 0.3);
    }

    return `<g transform="translate(${ox.toFixed(1)},${oy.toFixed(1)}) rotate(${rot.toFixed(1)},10,10) scale(${sc.toFixed(2)})">
      <path d="${d}" stroke="currentColor" stroke-width="1.6" fill="none" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"/>
    </g>`;
  }).join('\n');

  const dotElements = dots.map(({ x, y }) =>
    `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="1.4" fill="currentColor" opacity="0.75"/>`
  ).join('\n');

  // Unified visual theme palette (Aviation/Cyberpunk Amber theme)
  const colors = ['#ffb000', '#ff9900', '#ffaa00', '#e69500', '#ffc333'];
  ...
  return `<svg
    ...
    width="${size}"
    height="${size}"
    aria-label="Callsign Badge for ${callsign}"
    role="img"
    style="color:${color}; flex-shrink:0; display:inline-block; vertical-align:middle;"
  >
    <path d="${shape.outline}" stroke="currentColor" stroke-width="1.3" fill="none" opacity="0.65" stroke-linejoin="round" vector-effect="non-scaling-stroke"/>
    ${dotElements}
    ${glyphElements}
  </svg>`;
}
